...
export const useLoadUsers = (isFiltered) => {
    ...
    const getUsersFiltered = async ({pageParam = 1}) => {
        const filtersQuery = filteredQueryMaker(filters.countries, filters.roles, filters.programmingLanguages, filters.frameworks)
    ...
    return useInfiniteQuery([isFiltered ? 'usersFiltered' : 'users', id], isFiltered ? getUsersFiltered : getUsers, {
        getNextPageParam: (lastPage, allPages) => {
            return lastPage.next && lastPage.results.length === lastPage.next.limit ? allPages.length + 1 : undefined
        },
